...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/14 22:35:54 by gautierchau       #+#    #+#             */
/*   Updated: 2025/04/14 23:07:55 by gautierchau      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Constructors
BitcoinExchange::BitcoinExchange(std::string dbin): dbfile(dbin){
	this->filldb(dbin);
}
...
BitcoinExchange & BitcoinExchange::operator=(const BitcoinExchange &assign){
	this->dbfile = assign.dbfile;
	this->filldb(this->dbfile);
	return *this;
}
...
int		BitcoinExchange::ValueChecker(std::string input)
{
	...
	if (value < 0)
		throw NegVal();
	if (!isstrdigit(input) || value > 1000)
		throw InvalidValue();
	return 0;
}
...
void	BitcoinExchange::Convert(std::string input)
{
	...
	double res = iter->second * atof(in_value.c_str());
